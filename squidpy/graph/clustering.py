...
def prepare_data_for_clustering(adata: AnnData) -> None:
    """
    Do all data transformations that are independent of the ratio of the spatial and feature graphs and the degree \
...
def compute_louvain_on_joined_connectivities(adata: AnnData, alpha: float, nr_steps: int) -> None:
    """
    Compute louvain clusters based on a weighted sum of a spatial connectivity graph and a connectivity graph \
    ...
    joined_graph = alpha * adata.obsp["feature_connectivities"] + (1 - alpha) * adata.obsp["spatial_connectivity"]
...
def plot_louvain_on_graph(adata: AnnData) -> None:
    """
    Plot the computed louvain cluster assignments on top of the spatial graph.
...
def plot_louvain_on_umap(adata: AnnData) -> None:
    """
    Plot the computed louvain cluster assignments on top of the UMAP of the, potentially processed, node features.
