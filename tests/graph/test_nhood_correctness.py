...
def _reference_nhood_enrichment(
    ...
    *,
    ...
    :func:`squidpy.gr._nhood._permutation_moments_counts` exactly.
    """
...
def test_counts_match_reference_tiny(adata_tiny: AnnData):
    ...
    result = nhood_enrichment(adata_tiny, cluster_key=_CK, n_perms=20, rng=0, copy=True)
...
def test_zscore_independent_of_n_jobs(adata_tiny: AnnData, normalization: str):
    ...
    many threads run. ``'none'`` goes through the int64 kernel, where integer addition is
    associative and the z-score is therefore bit-identical for any thread count. The normalized
    modes divide by a row sum / conditional denominator, so they accumulate in float64 and match
    only to rounding (measured <= 1e-14 relative); the tolerance below leaves several orders of
...
def test_self_enrichment_positive_for_clustered_graph():
    ...
    result = nhood_enrichment(adata, cluster_key=_CK, n_perms=200, rng=0, n_jobs=1, copy=True)
