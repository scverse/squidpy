...
def test_cluster_auto_k_not_inplace_returns_a_copy_and_leaves_the_input_alone():
    adata = AnnData(make_blobs())
    out = cluster_auto_k(adata, (2, 4), max_runs=3, rng=0, inplace=False)
...
def test_cluster_auto_k_is_reproducible_from_the_rng():
    adata = AnnData(make_blobs())
    first = cluster_auto_k(adata, (2, 4), max_runs=3, rng=0, inplace=False)
    second = cluster_auto_k(adata, (2, 4), max_runs=3, rng=0, inplace=False)
...
def test_cellcharter_forwards_model_params():
    ...
    kwargs = {"distance": 1, "n_clusters": 3, "rng": 0, "inplace": False}
