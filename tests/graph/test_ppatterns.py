...
def test_spatial_autocorr_reproducibility(dummy_adata: AnnData, n_jobs: int, mode: str):
    ...
    assert "pval_norm_fdr_bh" in dummy_adata.uns[UNS_KEY]
    # test pval_norm same
    np.testing.assert_array_equal(df_1["pval_norm"].values, df_2["pval_norm"].values, rtol=1e-10, atol=1e-10)
    np.testing.assert_array_equal(df_1["var_norm"].values, df_2["var_norm"].values, rtol=1e-10, atol=1e-10)
    assert dummy_adata.uns[UNS_KEY].columns.shape == (4,)
