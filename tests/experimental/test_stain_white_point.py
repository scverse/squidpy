...
def test_recovers_white_point(chunked: bool) -> None:
    ...
    bg = estimate_white_point(_da(values, chunked=chunked))
    assert bg.shape == (3,)
...
def test_blank_image_raises() -> None:
    ...
    with pytest.raises(StainFittingError, match="non-positive"):
        estimate_white_point(_da(black, chunked=False))


def test_bad_percentile_raises() -> None:
    with pytest.raises(ValueError, match="percentile"):
        estimate_white_point(_da(np.ones((3, 8, 8)), chunked=False), percentile=0.0)
