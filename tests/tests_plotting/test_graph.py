...
class TestLigrec(PlotTester, metaclass=PlotTesterMeta):
    ...
    def test_invalid_alpha(self, ligrec_result: NamedTuple):
        with pytest.raises(ValueError, match=r"Expected `alpha`"):
            pl.ligrec(ligrec_result, alpha=1.2)

    def test_invalid_clusters(self, ligrec_result: NamedTuple):
    ...
    def test_all_interactions_empty(self, ligrec_result: NamedTuple):
    ...
    def test_plot_source_clusters(self, ligrec_result: NamedTuple):
        src_cls = ligrec_result.pvalues.columns.get_level_values(0)[0]
        pl.ligrec(ligrec_result, src_clusters=src_cls)

    def test_plot_target_clusters(self, ligrec_result: NamedTuple):
        tgt_cls = ligrec_result.pvalues.columns.get_level_values(1)[0]
        pl.ligrec(ligrec_result, tgt_clusters=tgt_cls)

    def test_plot_remove_empty_interactions(self, ligrec_result: NamedTuple):
        tmp = deepcopy(ligrec_result)
        tmp.pvalues.values[:2, :] = np.nan
        pl.ligrec(tmp, remove_empty_interactions=True)

    def test_plot_dendrogram(self, ligrec_result: NamedTuple):
        pl.ligrec(ligrec_result, dendrogram=True)

    def test_plot_swap_axes(self, ligrec_result: NamedTuple):
        pl.ligrec(ligrec_result, swap_axes=True)

    def test_plot_swap_axes_dedrogram(self, ligrec_result: NamedTuple):
        pl.ligrec(ligrec_result, swap_axes=True, dendrogram=True)

    def test_plot_alpha(self, ligrec_result: NamedTuple):
        pl.ligrec(ligrec_result, alpha=1)

    def test_plot_cmap(self, ligrec_result: NamedTuple):
        pl.ligrec(ligrec_result, cmap="inferno")

    def test_plot_kwargs(self, ligrec_result: NamedTuple):
