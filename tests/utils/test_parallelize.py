...
def test_parallelize_loky(func, n_jobs):
    ...
    tracker_pname = "joblib.externals.loky.backend.resource_tracker"
    processes = final_children - initial_children
    processes = {psutil.Process(pid) for pid in processes}
    processes = {p for p in processes if not any(tracker_pname in cl for cl in p.cmdline())}
    if n_jobs > 1:  # expect exactly n_jobs
        assert len(processes) == n_jobs, f"Unexpected new processes created: {processes}"
    else:  # some functions use the main process others use a new process
        assert len(processes) <= 1, f"Unexpected new processes created: {processes}"
