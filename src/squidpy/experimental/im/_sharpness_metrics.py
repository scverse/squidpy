...
def laplacian_variance(block: np.ndarray) -> np.ndarray:
...
def pop_variance(block: np.ndarray) -> np.ndarray:
    """Population variance of pixel intensities."""
    b = to_f32_2d(block)
    return np.array([[float(np.var(b))]], dtype=np.float32)


def fft_high_freq_mask(h: int, w: int) -> np.ndarray:
...
def fft_high_freq_energy(block: np.ndarray) -> np.ndarray:
    x = to_f32_2d(block).astype(np.float64, copy=False)
    m = float(x.mean())
    s = float(x.std())
    ...
    mask = fft_high_freq_mask(x.shape[0], x.shape[1])
...
def haar_wavelet_energy(block: np.ndarray) -> np.ndarray:
