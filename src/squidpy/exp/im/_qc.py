...
def _detect_tissue_rgb(
    img_da, tile_indices: np.ndarray, tiles_y: int, tiles_x: int, ty: int, tx: int
...
def detect_tissue(
    sdata, image_key: str, scale: str = "scale0", tile_size: Literal["auto"] | tuple[int, int] = "auto"
) -> AnnData:
    """
    ...
    logg.info("Preparing sharpness metrics calculation.")
    ...
    tile_indices, obs_names, pixel_bounds = _create_tile_indices_and_bounds(tiles_y, tiles_x, ty, tx, H, W)
...
def _detect_sharpness_outliers(
    X: np.ndarray, method: str = "iqr", tissue_mask: np.ndarray | None = None, var_names: list | None = None
) -> np.ndarray:
    """
...
def _detect_tenengrad_tissue_outliers(
    X: np.ndarray, tissue_mask: np.ndarray | None = None, var_names: list | None = None
) -> np.ndarray:
    """
...
def _detect_outliers_pvalue(
    X: np.ndarray, tissue_mask: np.ndarray | None = None, var_names: list | None = None, alpha: float = 0.05
...
def _make_tiles(
    arr_yx: da.Array,
    tile_size: Literal["auto"] | tuple[int, int],
) -> tuple[int, int]:
    """
...
def _create_tile_indices_and_bounds(
    tiles_y: int, tiles_x: int, tile_size_y: int, tile_size_x: int, image_height: int, image_width: int
...
def _create_tile_polygons(
    scores: np.ndarray, tile_size_y: int, tile_size_x: int, image_height: int, image_width: int
...
def qc_sharpness(
    ...
    image_key: str,
    scale: str = "scale0",
    metrics: SHARPNESS_METRICS | list[SHARPNESS_METRICS] = [
        SHARPNESS_METRICS.TENENGRAD,
        SHARPNESS_METRICS.VAR_OF_LAPLACIAN,
    ],
    ...
    print("")
    logg.info("Calculating sharpness metrics.")
    ...
    for metric in metrics_to_compute:
        ...
        if metric_name == "tenengrad":
            ...
            sharp_field = da.map_overlap(_variance_numba, gray_rechunked, depth=0, boundary="reflect", dtype=np.float32)
