...
def _co_occurrence_rs(spatial: NDArrayA, original_clust: pd.Series, interval: NDArrayA) -> NDArrayA | None:
    from scstat_rs import co_occur_count

    clust = original_clust.cat.codes.astype(np.int32)

    co_occur_3d = co_occur_count(spatial[:, 0], spatial[:, 1], interval, clust)
...
def co_occurrence(
    ...
    if use_rust and not find_spec("scstat_rs"):
        ...
        raise ImportError(
            "scstat_rs not found. Please install scstat_rs to use the rust version of co-occurrence function: `pip install scstat-rs`"
        )
    ...
    if use_rust:
        out = _co_occurrence_rs(spatial, original_clust, interval)
        ...
        print(
            "Using python implementation of co-occurrence function. For faster computation, consider using the rust implementation by setting `use_rust=True`"
        )
        spatial_x = spatial[:, 0]
        spatial_y = spatial[:, 1]
