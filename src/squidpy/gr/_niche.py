...
def calculate_niche(
    ...
    *,
    ...
    n_components
        ...
        Required if flavor == `{fla.CELLCHARTER.s!r}`.
    random_state
        Removed. Passing it raises a :class:`TypeError`; use `seed` instead.
    seed
    ...
    if random_state is not None:
        raise TypeError(
            "'random_state' is no longer supported, provide 'seed' instead. Unlike "
            "'random_state', 'seed' seeds a `numpy.random.Generator` and derives an "
            "independent seed per library and per resolution, so results differ from "
            "those obtained with `random_state`."
        )
