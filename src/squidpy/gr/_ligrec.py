...
from squidpy._utils import NDArrayA, _get_n_cores, deprecated_params, numba_threads, spawn_generators
...
def _score_permutations(
    ...
    counts = np.zeros((n_inter, n_cpairs), dtype=np.int64)
...
def _analysis(
    ...
    with (
        ...
        ProgressBar(total=n_perms, unit="permutation", disable=not show_progress_bar) as progress,
    ):
