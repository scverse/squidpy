...
def gpu_dispatch(rapids_module: str, rapids_func_name: str | None = None) -> Callable[[F], F]:
    """
    Decorator to dispatch to rapids-singlecell GPU implementation when device="gpu".

    Automatically:
    1. Resolves effective device (from arg or settings)
    2. If GPU: imports rapids function, filters kwargs to match its signature, calls it
    3. If CPU: proceeds with the original squidpy implementation
    ...
    -------
    Callable
        Decorated function that dispatches to GPU or CPU implementation.
    ...
    def decorator(func: F) -> F:
        ...
        def wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            # Bind arguments to get a complete view of all parameters
            sig = inspect.signature(func)
            try:
                bound = sig.bind(*args, **kwargs)
            except TypeError:
                # If binding fails, fall back to original function to get proper error
                return func(*args, **kwargs)

            bound.apply_defaults()
            all_args = dict(bound.arguments)

            # Extract and resolve device
            device = all_args.pop("device", None)
            effective_device = resolve_device_arg(device)

            if effective_device == "gpu":
                # Import rapids function
                module = importlib.import_module(rapids_module)
                func_name = rapids_func_name if rapids_func_name is not None else func.__name__
                rapids_func = getattr(module, func_name)

                # Get rapids function's accepted parameters
                rapids_sig = inspect.signature(rapids_func)
                rapids_params = set(rapids_sig.parameters.keys())

                # Filter to only parameters accepted by rapids function
                filtered_args = {k: v for k, v in all_args.items() if k in rapids_params}

                return rapids_func(**filtered_args)

            # CPU path: call original function without device
