...
- Add `sort_order` to {func}`squidpy.pl.spatial_scatter`, enabled by default, to draw higher continuous color values on top. Set `sort_order=False` to retain observation order.
