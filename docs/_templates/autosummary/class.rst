...
{#- A `TypedDict` subclasses `dict` and a `NamedTuple` subclasses `tuple`, so autodoc reports
    each container's whole API as inherited members. None of it is the documented surface, so
    hide it where it is inherited, not where a class defines its own. #}
{%- set dict_api = ['clear', 'copy', 'fromkeys', 'get', 'items', 'keys', 'pop',
                    'popitem', 'setdefault', 'update', 'values'] %}
{%- set tuple_api = ['count', 'index'] %}
{%- set inherited_api = (dict_api + tuple_api) | select('in', inherited_members) | list %}
