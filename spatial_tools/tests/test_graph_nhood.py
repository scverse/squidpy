...
def test_cluster_centrality_scores():
    ...
    assert (
        ...
        == adata.uns["cluster_centrality_scores"].shape[0]
    )
    assert adata.uns["cluster_centrality_scores"]["cluster"].dtype == np.dtype("O")
    assert adata.uns["cluster_centrality_scores"][
        "degree centrality"
...
def test_cluster_interactions():
    ...
    assert (
        len(adata.obs["louvain"].unique())
        == adata.uns["cluster_interactions"][0].shape[0]
    )
    assert (
        len(adata.obs["louvain"].unique())
        == adata.uns["cluster_interactions"][0].shape[1]
    )
    assert type(adata.uns["cluster_interactions"][1]) == list
    assert len(adata.obs["louvain"].unique()) == len(
        adata.uns["cluster_interactions"][1]
    )
